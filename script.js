...
const dataPegawai = [
    {
        ...
        umur: 39,
        gred: 'N44',
        jawatan: 'Penolong Pegawai Tadbir Kanan',
        lokasi: 'HQ Putrajaya',
        tarikhLantik: '2010-03-15',
        kelayakan1: 'Ijazah Sarjana Muda Pentadbiran Awam',
        history: {
            'c9': { date: '2010-03-15', status: 'STANDARD' },
            'g10': { date: '2018-06-01', status: 'STANDARD' }
        }
    },
    {
        nama: 'Siti Sarah binti Raissuddin',
        ic: '900505-10-5588',
        umur: 34,
        gred: 'W41',
        jawatan: 'Akauntan',
    ...
    },
    {
        ...
        history: {
            ...
            'g10': { date: '2010-12-01', status: 'STANDARD' },
            'g12': { date: '2018-04-15', status: 'KUP' }
        }
    },
    {
        ...
        history: {
            ...
            'g10': { date: '2000-03-01', status: 'STANDARD' },
            'g12': { date: '2006-05-15', status: 'STANDARD' },
            'g13': { date: '2011-01-01', status: 'STANDARD' },
            'g14': { date: '2015-06-01', status: 'STANDARD' },
    ...
    },
    {
        ...
        history: {
            ...
            'g10': { date: '2020-01-01', status: 'KUP' }
        }
    },
    {
        nama: 'David Teoh',
        ...
        umur: 42,
        gred: 'F48',
        jawatan: 'Pegawai Teknologi Maklumat Kanan',
        lokasi: 'HQ Putrajaya',
        tarikhLantik: '2006-07-15',
        kelayakan1: 'Ijazah Sarjana Muda Sains Komputer',
        history: {
            'c9': { date: '2006-07-15', status: 'STANDARD' },
            'g10': { date: '2013-11-20', status: 'STANDARD' },
            'g12': { date: '2021-03-01', status: 'FLEKSI' }
        }
    },
    {
    ...
    },
    {
        ...
        history: {
            ...
            'g10': { date: '2013-01-01', status: 'KUP' }
        }
    },
    {
        nama: 'Tan Mei Ling',
    ...
    },
    {
        ...
        history: {
            ...
            'g10': { date: '1998-01-15', status: 'STANDARD' },
            'g12': { date: '2004-03-01', status: 'STANDARD' },
            'g13': { date: '2010-06-01', status: 'STANDARD' },
            'g14': { date: '2016-01-01', status: 'STANDARD' },
            'jusa-c': { date: '2021-01-01', status: 'KUP' }
        }
    }
];
        ...
        tunjukConfirmation(
            ...
            () => {
                ...
                gradeIds.forEach(id => {
                    ...
                    const container = dateInput ? dateInput.nextElementSibling : null;
                    if (container) {
                        const activeBtns = container.querySelectorAll('[class*="bg-"]');
...
statusButtons.forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        // Grade Logic: Check if this is the latest grade
        const gradeIds = ['c9', 'g10', 'g12', 'g13', 'g14', 'jusa-c', 'jusa-b', 'jusa-a'];
        let latestGradeId = null;

        // Find the latest filled grade
        for (let i = 0; i < gradeIds.length; i++) {
            const dateInput = document.getElementById(`date-${gradeIds[i]}`);
            if (dateInput && dateInput.value) {
                latestGradeId = gradeIds[i];
            }
        }

        const currentGradeId = container.previousElementSibling.id.replace('date-', '');
        const isLatest = (currentGradeId === latestGradeId);

        // Toggle Logic
        if (isLatest) {
            // MULTI-SELECT for Latest
            ...
            // 2. Activate clicked (Radio behavior: cannot deselect self by clicking again, or can? Let's allow toggle off too)
            if (this.classList.contains('text-white')) {
                this.classList.remove(`bg-${baseColor}-500`, 'text-white');
                this.classList.add(`text-${baseColor}-600`, `hover:bg-${baseColor}-50`);
            } else {
