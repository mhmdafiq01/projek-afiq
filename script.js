...
function tukarTab(idTab, elemen) {
    ...
    const blokLogAktiviti = document.getElementById('kandungan-log-aktiviti');
    if (blokLogAktiviti) blokLogAktiviti.classList.add('hidden');
    ...
    } else if (idTab === 'laman-log') {
        // Keselamatan: Hanya Admin boleh akses
        if (currentUser && currentUser.role === 'admin') {
            if (blokLogAktiviti) {
                blokLogAktiviti.classList.remove('hidden');
                lucide.createIcons();
                if (typeof renderLogAktiviti === 'function') {
                    renderLogAktiviti();
                }
            }
        } else {
...
// modalConfirm fetched dynamically to verify existence
...
function tunjukConfirmation(mesej, action) {
    ...
    if (!modal) {
        console.error('Error: Modal Confirm element not found.');
        alert(mesej); // Fallback if modal broken
        if (action) {
            if (confirm("Teruskan tindakan ini?")) action();
        }
        return;
    }

    if (msg) msg.innerHTML = mesej;
    actionToConfirm = action;

    // Tunjuk modal
    modal.classList.remove('hidden');
    modal.style.display = 'flex'; // Force display
    modal.style.zIndex = '9999'; // Force on top
...
function tutupConfirmation() {
    // Re-fetch dynamically to be safe
    const modal = document.getElementById('modal-confirm');
    if (modal) {
        modal.classList.add('hidden');
        modal.style.display = 'none'; // Reset handling
...
if (btnBatalConfirm) {
    btnBatalConfirm.addEventListener('click', (e) => {
        e.preventDefault();
        tutupConfirmation();
    });
}
...
if (btnTeruskanConfirm) {
    btnTeruskanConfirm.addEventListener('click', (e) => {
        e.preventDefault();
...
const modalConfirmDynamic = document.getElementById('modal-confirm');
if (modalConfirmDynamic) {
    modalConfirmDynamic.addEventListener('click', (e) => {
        if (e.target === modalConfirmDynamic) tutupConfirmation();
    });
}
...
// (Declarations moved to Unified Module section)
...
// Setup Carian Profil
// Setup Carian Profil & Gred removed (Replaced by Unified Search)

// --- 11. MODUL PENGEMASKINIAN (UNIFIED) ---

// Elements - UNIFIED Search & Inputs
const inputCarianUtama = document.getElementById('carian-utama-kemaskini');
const hasilCarianUtama = document.getElementById('hasil-carian-utama');
...
// Grade Inputs Display (Hidden variables for compatibility or simple referencing)
const displayNamaGred = { innerText: '', classList: { add: () => { }, remove: () => { } } }; // Dummy stub as we don't use these specific separate displays anymore
const displayKpGred = { innerText: '' };

// Buttons
const btnTunjukUnified = document.getElementById('btn-tambah-unified');
const btnSimpanUnified = document.getElementById('btn-simpan-unified');
const btnHapusUnified = document.getElementById('btn-hapus-unified');
const btnKosongkanUnified = document.getElementById('btn-kosongkan-unified');

let pegawaiSedangDieditIndex = null;
let pegawaiSedangDieditGredIndex = null; // Sync with above

// --- UNIFIED SEARCH SETUP ---
if (inputCarianUtama && hasilCarianUtama) {
    setupCarianDropdown(inputCarianUtama, hasilCarianUtama, (pegawai, index) => {
        // Load BOTH Profile and Grade data
        isiBorangProfil(pegawai, index);
        isiBorangGred(pegawai, index);
        tunjukPopup(`Memuatkan data: ${pegawai.nama}`);
    });
}

function isiBorangProfil(pegawai, index) {
    pegawaiSedangDieditIndex = index;
    pegawaiSedangDieditGredIndex = index; // Keep synced
...
function isiBorangGred(pegawai, index) {
    // Populate Grade Inputs (Improved Logic for new C-prefix IDs)
    const history = pegawai.history || {};
    // Ensure these match the HTML IDs: date-c9, date-c10, etc.
    ...
    gradeIds.forEach(id => {
        ...
        if (dateInput) {
            dateInput.value = '';
            // Handle buttons
            const container = dateInput.nextElementSibling;
            if (container) {
        ...
        // Fill if exists
        if (history[id] && dateInput) {
            dateInput.value = history[id].date || '';
            const savedStatus = history[id].status; // e.g. "KUP" or "KUP, FLEKSI"
            ...
            if (savedStatus) {
                ...
                if (container) {
                    const buttons = container.querySelectorAll('button');
                    buttons.forEach(btn => {
                        if (savedStatus.includes(btn.dataset.status)) {
                            const statusType = btn.dataset.status;
                            const baseColor = getColorName(statusType);
...
function kosongkanBorangGred() {
    // This is now generally handled by the Unified Reset button listener,
    // but kept empty or minimal to prevent reference errors if called elsewhere.
}
...
// 1. Reset / Kosongkan
if (btnKosongkanUnified) {
    btnKosongkanUnified.addEventListener('click', () => {
        // Reset Variables
        ...
        inputKp.value = '';
        inputJawatan.value = '';
        inputLokasiUpdate.value = '';
        inputTarikhLantik.value = '';
        inputKelayakan1.value = '';
        inputKelayakan2.value = '';
        inputKelayakan3.value = '';
        inputCarianUtama.value = '';
        ...
        tunjukPopup('Borang telah dikosongkan.');
    });
}

// 2. Tambah Baru
if (btnTunjukUnified) {
    btnTunjukUnified.addEventListener('click', (e) => {
        e.preventDefault();

        const valNama = inputNama.value.trim();
        const valKp = inputKp.value.trim();

        if (!valNama || !valKp) {
            tunjukPopup('Sila isi Nama dan No. KP.', 'negatif');
            return;
        }

        const isDuplicate = dataPegawai.some(p => p.ic === valKp);
        if (isDuplicate) {
            tunjukPopup('Gagal: No. KP telah wujud.', 'negatif');
            return;
        }

        tunjukConfirmation(
            "Adakah anda pasti mahu menambah pegawai baru?",
            () => {
                const pegawaiBaru = {
                    nama: valNama,
                    ic: valKp,
                    ...
                    umur: 30,
                    gred: 'N/A', // Auto-calc ideally, assuming N/A for now
                ...
                // Add Grade History logic for New User? 
                // Currently user creates, then edits grades. 
                // Or we can save grades immediately if filled.
                // Let's safe grades immediately if filled.
                const gradeIds = ['c9', 'c10', 'c12', 'c13', 'c14', 'jusa-c', 'jusa-b', 'jusa-a'];
                gradeIds.forEach(id => {
                    const dateInput = document.getElementById(`date-${id}`);
                    ...
                    if (dateVal) {
                        ...
                        pegawaiBaru.history[id] = { date: dateVal, status: statusVal };
                    }
                });

                dataPegawai.push(pegawaiBaru);
                tambahLog('TAMBAH', 'Menambah pegawai baru: ' + pegawaiBaru.nama);
                btnKosongkanUnified.click(); // Reset form
                tunjukPopup('Pegawai baru ditambah!');
                renderDirektori();
            }
        );
    });
...
if (btnSimpanUnified) {
    btnSimpanUnified.addEventListener('click', (e) => {
        e.preventDefault();

        if (pegawaiSedangDieditIndex === null) {
            tunjukPopup('Sila cari pegawai dahulu.', 'negatif');
            return;
        }

        tunjukConfirmation(
            "Simpan perubahan Profil dan Gred?",
            () => {
                ...
                gradeIds.forEach(id => {
                    ...
                    // We only update if there's a date or we explicitly want to clear? 
                    // Should we clear if date is empty? Yes.
                    ...
                    if (dateVal) {
                        p.history[id] = { date: dateVal, status: statusVal };
                    } else {
                        // If date is cleared, remove history key? or keep empty?
                        // Better to keep structure clean
                        if (p.history[id]) delete p.history[id];
                    }
                });

                tunjukPopup('Data berjaya dikemaskini!');
                tambahLog('KEMASKINI', 'Mengemaskini data pegawai: ' + p.nama);
                renderDirektori();
            }
        );
    });
...
// 4. Hapus
if (btnHapusUnified) {
    btnHapusUnified.addEventListener('click', (e) => {
        e.preventDefault();
        ...
        // Debug to confirm logic flow (User requested fix, this confirms click registers)
        // alert('Debug: Hapus button clicked. Attempting confirmation.'); 
        tunjukConfirmation(
            "<span class='text-red-500 font-bold'>AMARAN!</span> Data akan dipadam kekal.",
            () => {
                const namaHapus = dataPegawai[pegawaiSedangDieditIndex].nama;
                dataPegawai.splice(pegawaiSedangDieditIndex, 1);
                tambahLog('HAPUS', 'Memadam rekod pegawai: ' + namaHapus);
                btnKosongkanUnified.click();
                tunjukPopup('Rekod dibuang.');
                renderDirektori();
            }
        );
    });
...
// ==========================================
// 14. MODUL LOG AKTIVITI (NEW)
// ==========================================
let dataLogAktiviti = JSON.parse(localStorage.getItem('dataLogAktiviti')) || [];

// Fungsi Tambah Log
function tambahLog(aksi, butiran) {
    const logBaru = {
        timestamp: new Date().toISOString(),
        aksi: aksi, // 'TAMBAH', 'KEMASKINI', 'HAPUS'
        butiran: butiran,
        admin: (currentUser && currentUser.username) ? currentUser.username : 'Admin'
    };

    // Simpan ke array (Top = Terkini)
    dataLogAktiviti.unshift(logBaru);

    // Limit log (Optional, simpan 50 terkini)
    if (dataLogAktiviti.length > 50) dataLogAktiviti.pop();

    // Simpan ke LocalStorage
    localStorage.setItem('dataLogAktiviti', JSON.stringify(dataLogAktiviti));
...
function renderLogAktiviti() {
    const listContainer = document.getElementById('log-aktiviti-list');
    const filterInput = document.getElementById('filter-tarikh-log');
    ...
    // Filter Logic
    let filteredLogs = dataLogAktiviti;
    if (filterInput && filterInput.value) {
        const selectedDate = new Date(filterInput.value).toDateString();
        filteredLogs = dataLogAktiviti.filter(log => {
    ...
    if (filteredLogs.length === 0) {
        const msg = (filterInput && filterInput.value) ? "Tiada aktiviti pada tarikh ini." : "Tiada aktiviti direkodkan.";
        listContainer.innerHTML = `
            <div class="relative">
                 <span class="absolute -left-[43px] w-5 h-5 rounded-full border-4 border-white bg-slate-300"></span>
    ...
    filteredLogs.forEach((log) => {
        ...
        let judulAksi = log.aksi;

        if (log.aksi === 'TAMBAH') { colorClass = 'emerald'; judulAksi = "Rekod Ditambah"; }
        if (log.aksi === 'KEMASKINI') { colorClass = 'blue'; judulAksi = "Rekod Dikemaskini"; }
        if (log.aksi === 'HAPUS') { colorClass = 'red'; judulAksi = "Rekod Dihapus"; }
        ...
        item.innerHTML = `
            <span class="absolute -left-[43px] w-5 h-5 rounded-full border-4 border-white bg-${colorClass}-500 shadow-sm shadow-${colorClass}-200"></span>
...
function kosongkanLog() {
    tunjukConfirmation("Padam SEMUA rekod log aktiviti?", () => {
        dataLogAktiviti = [];
        localStorage.removeItem('dataLogAktiviti');
        renderLogAktiviti();
        tunjukPopup('Log aktiviti telah dipadam.');
    });
}

// Initial Render bila page load
renderLogAktiviti();
